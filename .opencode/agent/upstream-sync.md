---
description: 从上游仓库拉取最新提交，合并到当前分支，然后汇报上游改动简报。
mode: subagent
permission:
  question: allow
  bash:
...
6. 解决不了的冲突不要硬猜：abort 恢复原状，用 question 工具给出可行方案让用户选择，每个方案说明冲突文件、双方意图、取舍和风险。
...
**二、上游改动简报**：按主题分组（如子代理修复 / 网络重试 / 新功能 / 其他），用通俗中文说清楚"改了什么、解决了什么问题"，不直译英文提交信息；每条一到两句，琐碎改动（文档翻译、chore: generate、版本同步）合并一条带过

汇报完后用 question 工具问用户是否推送到 origin，同意后 `git push origin <branch>`。推送前必须经过这一步确认。
